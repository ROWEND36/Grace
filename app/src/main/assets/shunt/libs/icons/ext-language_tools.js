...
(function() {
    ...
    function insert(list,val,index){
        var head = list.slice(0,index)
        var tail = list.slice(index,list.length)
        head.push(val)
            ...
            completer.getCompletions(editor, session, pos, prefix, function(err, results) {
                if (!err && results) {
                    ...
                    for (i = 0; i < matches.length; i++) {
                        if (!matches[i].priority || (completer.priority && completer.priority > matches[i].priority)) {
                            break                            
                        }
                    }
                    matches = insert(matches, { results: results, priority: completer.priority }, i)
                    for (i = 0; i < matches.length; i++) {
                        compiled = compiled.concat(matches[i].results)
                    }
                }
